...
RSpec.describe Admin::TPIPagesController, type: :controller do
  let(:admin) { create(:admin_user) }
  let_it_be(:page) { create(:tpi_page) }
  ...
  describe 'POST create' do
    context 'with valid params' do
      ...
      subject { post :create, params: {tpi_page: valid_params} }

      it 'creates a new TPI Page' do
        expect { subject }.to change(TPIPage, :count).by(1)
      ...
      it 'redirects to the created Page' do
        expect(subject).to redirect_to(admin_tpi_page_path(last_page_created))
      end
    end

    context 'with invalid params' do
      let(:invalid_params) { attributes_for(:tpi_page, title: nil) }

      subject { post :create, params: {tpi_page: invalid_params} }

      it { is_expected.to be_successful }

      it 'invalid_attributes do not create a Page' do
        expect { subject }.not_to change(TPIPage, :count)
      end
    end
  end

  describe 'PATCH update' do
    let!(:page_to_update) { create(:tpi_page) }

    context 'with valid params' do
      let(:valid_update_params) { {title: 'title was updated'} }

      subject { patch :update, params: {id: page_to_update.id, tpi_page: valid_update_params} }
      ...
      it 'redirects to the updated Page' do
        expect(subject).to redirect_to(admin_tpi_page_path(page_to_update))
      end
    end
  end

  describe 'DELETE destroy' do
    let!(:page_to_delete) { create(:tpi_page) }
    ...
    context 'with valid params' do
      ...
      it 'deletes Page' do
        ...
        expect { subject }.to change { TPIPage.count }.by(-1)
        expect(TPIPage.find_by_id(id_to_delete)).to be_nil
