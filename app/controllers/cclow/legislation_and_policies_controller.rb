    ...
    def index
      ...
      if params[:fromDate]
        @legislations = CCLOW::LegislationDecorator
          .decorate_collection(Legislation.plubished.where('updated_at >= ?', params[:fromDate]))
      elsif params[:ids]
        ids = params[:ids].split(',').map(&:to_i)
        @legislations = CCLOW::LegislationDecorator.decorate_collection(Legislation.plubished.find(ids))
